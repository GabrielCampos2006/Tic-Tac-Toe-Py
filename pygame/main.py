...
#as imagens (Corrigidas: X e O agora carregam as imagens corretas)
VELHA = pygame.image.load('Tic-Tac-Toe-Py/pygame/assets/Board.png')
IMG_X = pygame.image.load('Tic-Tac-Toe-Py/pygame/assets/X.png')
IMG_O = pygame.image.load('Tic-Tac-Toe-Py/pygame/assets/O.png')
...
def desenhar_linha_vencedora(pos_inicial, pos_final):
    """Desenha um risco vermelho grosso sobre a jogada vitoriosa"""
...
#controle da interface do jogo, para saber em qual tela está e qual modo de jogo está sendo jogado
estado_atual = "MENU"
modo_de_jogo = None   
resultado_texto = ""  
simbolo_jogador = "X" 
...
def resetar_jogo():
    ...
    campo = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    campo_grafico = [[[None, None], [None, None], [None, None]], [[None, None], [None, None], [None, None]], 
[[None, None], [None, None], [None, None]]]
    mover = "X"
    tela.fill(cor_da_tela)
    tela.blit(VELHA, (64, 64))
...
while True:
    ...
    if estado_atual == "MENU":
        ...
        # Verifica se houve cliques nos botões do menu
        for evento in eventos:
    ...
    # Tela de jogando
        ...
        # Jogador humano faz a jogada se for o turno dele ou se o modo for manual
        if mover == simbolo_jogador or modo_de_jogo == "MANUAL":
            for evento in eventos:
                if evento.type == pygame.MOUSEBUTTONDOWN:
                    campo, mover = X_ou_O(campo, campo_grafico, mover)
                    
                    # Verifica se tem algum vencedor após a jogada
                    vencedor_atual = Vitoria(campo)
                    if vencedor_atual is not None:
                        # Pequeno atraso para o jogador conseguir ver o risco antes da caixa de fim de jogo aparecer
        ...
        # A Minimax ou Alfa-Beta faz a jogada assumindo o turno oposto ao humano
        elif mover != simbolo_jogador and modo_de_jogo in ["MINIMAX", "ALFA_BETA"]:
            pass 

    # Tela do fim de jogo, onde mostra o resultado da partida e dá a opção de voltar ao menu ou tentar novamente
    ...
    # Ambiente destinado a fazer testes automatizados, para verificar se o algoritmo minimax e alfa-beta estão funcionando corretamente
    elif estado_atual == "TESTE":
        tela.fill((30, 30, 30))
        titulo_logs = fonte_titulo.render("Teste Automatizado", True, (255, 255, 255))
        tela.blit(titulo_logs, (largura//2 - titulo_logs.get_width()//2, 50))
        
        bt_voltar_teste = desenhar_botao("Voltar ao Menu", 300, 750, 300, 50, (178, 34, 34), (220, 20, 60))
        
        for evento in eventos:
            if evento.type == pygame.MOUSEBUTTONDOWN:
                pos = pygame.mouse.get_pos()
                if bt_voltar_teste.collidepoint(pos):
                    estado_atual = "MENU"
